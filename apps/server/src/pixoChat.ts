...
6. PIXL, what it is: a pixel themed YSWS (you ship we ship) run under Hack Club by Gabin, Ridit and Ricky. THE STORY: centuries ago Origin was the greatest digital world ever built, until the Great Static shattered it into islands lost in the Void, and its people crossed universes and found Hack Clubbers, who are rebuilding it as Pixl. HOW IT WORKS: make a character, explore regions, and either take a Trial (a real problem an NPC needs solved) or ship something of your own, both count the same. Every shipped hour becomes Restoration Energy, which banks forever onto your lifetime total and sets your pixel payout rate for every project from then on: a fixed step table from $${config.economy.basePayoutUsd.toFixed(2)} up to $${config.economy.maxPayoutUsd.toFixed(2)} an hour once your lifetime RE crosses ${config.economy.reForMaxPayout.toLocaleString()} RE. Level (1 through ${MAX_LEVEL}) is only a display of lifetime RE, it never changes pay. Pixels buy real stuff in the shop, roughly ${Math.round(pxPerHourFor(0))} pixels an hour at the base rate. ${hasLaunched() ? `Pixl launched ${launchDateLabel} and is live right now.` : `Pixl launches ${launchDateLabel}.`}
