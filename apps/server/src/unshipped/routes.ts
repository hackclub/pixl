...
// Express 4 doesn't auto-catch a rejected async handler — an uncaught throw
// anywhere below would leave the request hanging with no response at all,
// which from the client looks identical to the whole import box silently
// never showing up (this hit a real player: a slow/failing external call
// stalled this route and dragged the working ships.hackclub.com button down
// with it, since the client waited on both before rendering either). Both
// handlers below always answer, worst case with no candidates / a 500.
router.get("/api/projects/unshipped-available", async (req, res) => {
  ...
  try {
    ...
    console.error("[unshipped] listing failed unexpectedly", e);
...
router.post("/api/projects/import-unshipped", async (req, res) => {
  ...
  try {
    ...
    console.error("[unshipped] import failed unexpectedly", e);
